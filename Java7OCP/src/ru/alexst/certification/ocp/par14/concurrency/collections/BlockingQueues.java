...
class TaskCreator implements Runnable {
	...
	public void run() {
		...
		while (doWork) {
			...
			if (added) {
				System.out.println(String.format("New task added with number %s. Number of tasks in queue - %s", counter, queue.size()));
			} else {
				System.out.println(String.format("Queue is full. Task with number %s is lost. Queue tasks number - %s", counter, queue.size()));
