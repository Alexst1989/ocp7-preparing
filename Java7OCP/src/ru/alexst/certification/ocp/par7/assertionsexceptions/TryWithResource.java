...
public class TryWithResource {
	...
	public static void main(String[] args)  {
		...
		try (Reader reader = new BufferedReader(new FileReader(file))) { // note the new syntax
			// read from file
		} catch (IOException e) { 
		...
		try (Reader reader =  new BufferedReader(new FileReader(file))) { //Without catch or finally block
			 // 
		...
		try (Reader reader =  new BufferedReader(new FileReader(file));
			 AutoCloseableClass myResource = new AutoCloseableClass()) { //More than one resource
			 // 
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
